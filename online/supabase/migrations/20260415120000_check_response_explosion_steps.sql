-- Cadena de explosión (regla desktop): dado + fichas == cara máxima → tiradas extra.
-- Igual que `ui/character_panel.py` / `core/dice.py` (roll_explosion_chain entrada).

drop function if exists public.submit_check_response(uuid, uuid, integer, integer, integer, text);

alter table public.check_responses
  add column if not exists explosion_steps jsonb not null default '[]'::jsonb;

comment on column public.check_responses.explosion_steps is
  'JSON array de {roll, tokens} por cada segmento post-tirada principal (orden de la cadena).';
...
begin
  ...
  -- Cadena desktop: segmentos 0..v_len-1 explotan; el último no.
  ...
  set
    current_tokens = v_rt.current_tokens - v_tokens_all,
...
comment on function public.submit_check_response(uuid, uuid, integer, integer, integer, text, jsonb) is
  'Respuesta de check con cadena opcional de explosión (desktop: dado+fichas=cara máxima). p_explosion_steps: [{roll,tokens}, ...].';

-- La firma de 6 args ya se eliminó arriba con DROP; no hacer REVOKE sobre ella (error 42883).
