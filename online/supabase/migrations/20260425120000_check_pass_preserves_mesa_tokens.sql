-- Checks y fichas en mesa:
-- · Éxito sin gastar fichas en la tirada (total natural/MOD): `current_tokens` igual.
-- · Éxito gastando fichas para alcanzar el CHECK: se descuenta solo lo gastado (toda la cadena).
-- · Fallo: +1 ficha; además se descuentan las fichas que se declararon gastadas en la tirada.
-- Explosión (20 / dado+fichas=cara máx): ya va en `p_explosion_steps` y en `v_tokens_all`.
...
begin
  ...
  set
    current_tokens = case
      when v_outcome = 'pass'::public.check_response_outcome then
        case
          when v_tokens_all = 0 then v_rt.current_tokens
          else v_rt.current_tokens - v_tokens_all
        end
      else v_rt.current_tokens - v_tokens_all + v_fail_token_reward
    end,
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'tokens_spent_total', v_tokens_all,
      'mesa_tokens_unchanged',
        (v_outcome = 'pass'::public.check_response_outcome and v_tokens_all = 0)
    )
  );
...
comment on function public.submit_check_response(uuid, uuid, integer, integer, integer, text, jsonb) is
  'Check con explosión desktop opcional. Éxito sin gastar fichas: saldo igual. Éxito gastando: descuenta solo lo gastado. Fallo: descuenta lo gastado +1 ficha.';
